...
document.getElementById('leaderBoardButton').onclick = async() => {
  ...
  let response = await axios.get(`http://54.161.13.80:3000/premium/leadership?page=${page}&limit=${limit}`, { headers: {"Authorization": token}});
    ...
    nextButton.onclick = async () => {
      ...
      page++;
      const response = await axios.get(`http://54.161.13.80:3000/premium/leadership?page=${page}&pageSize=${limit}`, {
...
const download = async() => {
  try{
  const token = localStorage.getItem('token');
  await axios.get('http://54.161.13.80:3000/user/download', { headers: {"Authorization" : token} })
  .then((response) => {
...
const downloadedFiles = async() => {
  ...
  const token = localStorage.getItem('token');
  const response = await axios.get('http://54.161.13.80:3000/downloadedFiles/all', { headers: {"Authorization" : token} })
...
window.addEventListener("DOMContentLoaded",async ()=>{
  try{
    ...
    if(decodedToken.isPremium){
        ...
        let response = await  axios.get("http://54.161.13.80:3000/expense/get-expenses", {
        headers: {"Authorization": token}})
  ...
  const obj = { cost,desc,cat}
    ...
    try {
        ...
        if(expenseId !== null){
            ...
            response = await axios.post(`http://54.161.13.80:3000/expense/update-expense/${expenseId}`,obj,{headers:{"Authorization":token}});
            expenseId=null;
            showExpenses(response.data.expense); 
        }
        else{    
            response = await axios.post("http://54.161.13.80:3000/expense/add-expense", obj,{headers:{"Authorization":token}});         
            showExpenses(response.data.expense); 
          } 
    }         
    catch (error) {
 ...
 document.getElementById('premium').onclick = async(e)=>{
   ...
   const response =await axios.get('http://54.161.13.80:3000/purchase/premiummembership',
   {headers:{"Authorization":token}});
   ...
   var options={
    ...
    "handler" : async function(response){
        await axios.post('http://54.161.13.80:3000/purchase/updatetransactionstatus',{
...
// show expenses with edit and delete button 
  function showExpenses(obj){
      ...
      dltbtn.onclick=()=>{
          const token  = localStorage.getItem('token');
          expenseList.removeChild(expense);
          axios.delete(`http://54.161.13.80:3000/expense/delete-expense/${obj._id.toString()}`,{headers:{"Authorization":token}});   
      }
